...
func HighlightContent(content string, area image.Rectangle, startLine, startCol, endLine, endCol int) string {
	...
	for y := startLine; y <= endLine && y < height; y++ {
		...
		// Find the last cell in the extraction range that holds any
		// content, including explicit spaces (renderers like glamour pad
		// rows with real space cells, so content usually reaches the full
		// width).
		lastCellX := -1
